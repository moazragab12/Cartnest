...
// Enhanced sidebar with no scrolling - content expands naturally
function setupEnhancedSidebar() {
  const sidebar = document.querySelector(".sidebar");
  if (!sidebar) return;

  // Make sure overflow is set to visible
  sidebar.style.overflow = "visible";

  // Remove any existing classes that might enable scrolling
  sidebar.classList.remove("needs-scroll");

  // Create container for sidebar if it doesn't exist (for styling/animation purposes)
  ...
  // Add listeners for detail elements open/close state for animation purposes only
  const detailsElements = sidebar.querySelectorAll("details");
  detailsElements.forEach((details) => {
    details.addEventListener("toggle", () => {
      // No scrollbar checks needed
    });
  });

  // Add simple animation when clicking on filter headers
  ...
  function adjustSidebarHeight() {
    // Simply ensure overflow is always visible
    sidebar.style.overflow = "visible";
  }
...
function setupClearFilters() {
  ...
  clearFiltersBtn.addEventListener("click", () => {
    ...
    }, 1500); // No need to scroll the sidebar anymore since it expands naturally
    // Just make sure it maintains visible overflow
    const sidebar = document.querySelector(".sidebar");
    if (sidebar) {
      sidebar.style.overflow = "visible";
    }
...
function setupEnhancedStickySidebar() {
  ...
  function handleStickyScroll() {
    ...
    if (scrollTop > sidebarTop - headerOffset) {
      ...
      const topOffset =
        scrollDirection === "down" && scrollSpeed > scrollThreshold
          ? headerOffset + 5
          : headerOffset;
