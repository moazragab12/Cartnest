    ...
    if (e.target && e.target.classList.contains("quantity-select")) {
      ...
      if (itemElement) {
        ...
        const unitPriceMatch =
          itemPriceElem.textContent.match(/\$(\d+(\.\d+)?)/);
...
function updateOrderSummary(totalItems, totalPrice) {
  ...
  summaryInfo.innerHTML = `
        <p><span class="row-label">Subtotal:</span> <span class="amount">$${subtotal.toFixed(
          2
        )}</span></p>
        ...
        <hr />
        <p class="total"><span class="row-label">Total:</span> <span class="amount">$${grandTotal.toFixed(
          2
        )}</span></p>
    `;
}
...
function updateDeliveryDate() {
  ...
  if (
    minDeliveryDate.getFullYear() !== today.getFullYear() ||
    maxDeliveryDate.getFullYear() !== today.getFullYear()
  ) {
